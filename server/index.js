...
function onConnection(socket) {
  ...
  socket.on('add-user', (data) => {
    ...
    if (Users.length == 1) {
      //if new user is only user online they will be the drawer by default
      nowDrawing = newUser;
  ...
  socket.on('winner', (data) => {
    //assigns winner, reset word counter and randomly assign the next drawer
    wordCounter = 0;
